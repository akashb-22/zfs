    ...
    const char *key, dmu_tx_t *tx)
{
	...
	if (cid < 0 || cid >= TZAP_CHUNK_SIZES) {
		/*
		 * Hint was set at create time but first add does not
		 * qualify.  Clear mz_flags so mzap_open() on remount
		 * does not misread plain MicroZAP entries as TinyZAP.
		 */
...
again:
	for (uint16_t i = start; i < zap->zap_m.zap_num_chunks; i++) {
		tzap_ent_phys_t *tze = (tzap_ent_phys_t *)((uint8_t *)
		    zap_m_phys(zap)->mz_chunk + (i * chunk));
